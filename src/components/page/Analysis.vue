  ...
  export default {
    ...
    methods:{
      ...
      async getData () {
          ...
          // console.log(res)
          this.list = res.data.map(item => ({date: item.date, posstrong: item.morePostive, posweak: item.positive,negstrong: item.moreNegative, negweak: item.negative}))
      ...
      take(){
        ...
        this.value[0] = this.list.map(function (item) { return item.posstrong;});
        this.value[1] = this.list.map(function (item) { return item.posweak;});
        this.value[2] = this.list.map(function (item) { return item.negstrong;});
        this.value[3] = this.list.map(function (item) { return item.negweak;});
      }
    }
  }
</script>
